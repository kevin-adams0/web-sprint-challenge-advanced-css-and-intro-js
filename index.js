...
function get20s(arr){
  ...
  for(let i = 0; i < arr.length; i++) {
    let born =  +arr[i].years.split(" ")[0] 
    let died =  +arr[i].years.split(" ")[2] 
...
function get20sRefactored(arr){
  let refactor = arr.filter((x)=> {
    return x.years.split(" ")[0] >= 1900 && x.years.split(" ")[2] <= 2000
  }) 
  return refactor;
}
